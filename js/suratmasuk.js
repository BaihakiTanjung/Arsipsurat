...
function showData(data){
	if(data == '' || data == null){
		...
		$("#txtTanggalterima").val(data.tanggal_terima);
...
function LoadData(){
	$("#surat_masuk tbody").html('Loading ...');
	$.ajax({
	 	type: 'post',
	 	data: {'MODE': 'AJAX', 'functions': 'LoadData'},
	 	success: function (returnValue, status){
	 		$("#surat_masuk tbody").html(returnValue);
	 	},
	 	error: function (e){
...
function Save(){
	...
	data.tanggal_terima=$("#txtTanggalterima").val();
