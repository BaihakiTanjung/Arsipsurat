...
if($_POST['MODE']=='AJAX'){
	switch ($_POST['functions']){
		...
		case 'Delete':
			$data=$_POST['data'];
			$sql="DELETE FROM surat_masuk WHERE no_agenda='".$data['no_agenda']."'";
			dbQuery($sql);
			echo "success";
			break;
		case 'showData':
			$data=$_POST['data'];
			$sql="SELECT * FROM surat_masuk WHERE no_agenda='".$data['no_agenda']."'";
			$rs=dbQuery($sql);
			$row=dbArray($rs);
		...
		case 'AddNew':
			...
			$sql="SELECT COUNT(*) FROM surat_masuk WHERE no_agenda='".$data['no_agenda']."'";
			$row=getone($sql);
			if($row==0){
				$sql="INSERT INTO surat_masuk (no_agenda,Id,jenis_surat,tanggal_kirim,tanggal_terima,no_surat,pengirim,perihal)"
					. "SELECT '".$data['no_agenda']."' as no_agenda"
					. ",'".$data['Id']."' as Id"
					. ",'".$data['jenis_surat']."' as jenis_surat"
					. ",'".$data['tanggal_kirim']."' as tanggal_kirim"
					. ",'".$data['tanggal_terima']."' as tanggal_terima"
		...
		case "Export":
			...
			$worksheet
					...
					->setCellValue('E'.$currentrow,'tanggal_terima')
			...
			while ($data = dbArray($exportdata)){
				$worksheet
						...
						->setCellValue('E'.$currentrow, $data['tanggal_terima'])
			...
			$filepath = "excel-files/SuratMasuk-".$_SESSION['UserID'].".xlsx";
